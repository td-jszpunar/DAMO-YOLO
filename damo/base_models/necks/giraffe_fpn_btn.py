...
def resize_to_match(x, ref):
    return F.interpolate(x.contiguous(), size=ref.shape[-2:], mode='nearest')


class GiraffeNeckV3(nn.Module):
    def __init__(
        self,
        depth=1.0,
        hidden_ratio=1.0,
    ...
    def forward(self, out_features):
        ...
        # node x3
        x13 = self.bu_conv13(x1)
        x13 = resize_to_match(x13, x0)
        x3 = torch.cat([x0, x13], 1)
        x3 = self.merge_3(x3)

        # node x4
        x24 = self.bu_conv24(x2)
        x34 = resize_to_match(x3, x1)
        x4 = torch.cat([x1, x24, x34], 1)
        x4 = self.merge_4(x4)

        # node x5
        x45 = resize_to_match(x4, x2)
        x5 = torch.cat([x2, x45], 1)
        x5 = self.merge_5(x5)

        # node x7
        x57 = self.bu_conv57(x5)
        x57 = resize_to_match(x57, x4)
        x7 = torch.cat([x4, x57], 1)
        x7 = self.merge_7(x7)
        ...
        x46 = self.bu_conv46(x4)
        x76 = self.bu_conv76(x7)
        x46 = resize_to_match(x46, x3)
        x76 = resize_to_match(x76, x3)
        x6 = torch.cat([x3, x46, x76], 1)
        x6 = self.merge_6(x6)
